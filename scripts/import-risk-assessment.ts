...
import { parseCSVLine, parseDateValue } from '../src/lib/data/csv-parser';
...
async function importRiskAssessmentCSV(csvFilePath: string) {
  try {
    ...
    console.log(`✅ Successfully imported ${inserted} risk assessment records!`);
    if (skipped > 0) {
...
// Run import
const csvFile = process.argv[2] || join(process.cwd(), 'Risk & Control Self-Assessment.csv');

importRiskAssessmentCSV(csvFile)
  .then(() => {
